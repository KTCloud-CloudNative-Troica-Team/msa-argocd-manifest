...
| # | 레포 명 | 내용 |
|---|---------|------|
| 7 | `msa-common-libs` | 기존 모노레포의 `common/`, `client-redis/`, `client-ses/`, 이벤트 스키마(Avro/Protobuf), 공통 예외/DTO. GitHub Packages로 배포 |
...
### 9.3 `notification.requested` 페이로드 스키마

```json
{
  "idempotencyKey": "<uuid>",
  "channel": "EMAIL" | "SLACK" | "SMS",
  "template": "ORDER_CONFIRMED_EMAIL" | "PASSWORD_RESET_EMAIL" | ...,
  "recipient": {
    "userId": "<uuid>",
...
스키마는 `msa-common-libs`의 `events/notification-requested.proto` (또는 Avro)로 정의 후 generated class 사용.
...
dependencies {
    implementation("com.troica.msa:common-libs:0.1.0")
}
```

로컬 개발 시에는 `~/.gradle/gradle.properties`에 `gpr.user` / `gpr.token` 등록.
...
2. 기존 모노레포에서 `common/`, `client-redis/`, `client-ses/` 모듈 이전
3. `events/` 디렉토리에 토픽 페이로드 스키마 (Protobuf 또는 Avro) 정의
4. §12.1 publish workflow 작성
5. `v0.1.0` 태그 push → GitHub Packages에 published 확인
