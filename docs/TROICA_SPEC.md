...
|---|---------|----------------------------|
| 1 | `msa-user-service` | 회원 정보 관리. 모노레포 `user/` 단독 (identification 폐기, auth 분리) |
| 2 | `msa-auth-service` | 회원 가입·인증·JWT 발급/검증. 모노레포 `auth/` + `auth-service/` (D1 결정 — 신규 레포) |
| 3 | `msa-product-service` | 상품 정보·카테고리 관리 |
| 4 | `msa-inventory-service` | 재고 조회·차감 + **Event Sourcing + Spring Batch worker** (Q7) |
| 5 | `msa-order-service` | 주문 생성 + **확장된 state machine** (Q6: payment/shipping 시뮬레이션) + Saga 오케스트레이션 + Outbox poller worker 분기 |
| 6 | `msa-api-gateway` | **BFF + Spring Cloud Gateway 혼합** (Q1 (c)). JWT 필터(`JwtHeaderCheckFilter`)로 보호 경로 인증 — `auth-service.checkValidity` gRPC 호출 |

> **archive**: ~~`msa-notification-service`~~ Q3로 폐기, GitHub UI에서 Archive 처리 (D6).
...
| 서비스 | HTTP | gRPC | DB 포트 | Redis 포트 | 비고 |
|--------|------|------|---------|-----------|------|
| product-service   | 8001 | 9001 | 7001 (product_db) | - | |
| order-service     | 8002 | 9002 | 7002 (order_db)   | - | worker pod (Outbox poller)는 `web-application-type=none` + `grpc.server.port=-1` |
| inventory-service | 8003 | 9003 | 7004 (inventory_db) | 7005 | worker pod (Spring Batch event-sourcing projection) 별도 K8s Deployment |
| user-service      | 8004 | 9004 | (별도 user_db, 신규) | - | gRPC 노출 — auth-service가 `:user` 라이브러리 사용 |
| auth-service      | 8005 | 9005 | 7007 (auth_db, 신규) | 7006 (auth refresh-token 캐시) | gRPC `AuthService.{SignUp, SignIn, CheckValidity}` |
| api-gateway       | 8100 | -    | - | - | 외부 진입점. WebFlux + JwtHeaderCheckFilter |

> Kafka는 single broker `localhost:7003` 공유 (KRaft 모드, 모노레포 `container-compose.yaml` 기준).
> Phase 4에서 user-service / auth-service / notification 폐기를 반영. 8005는 ~~notification~~ → **auth-service**로 재할당.
...
## 9. Kafka 토픽 카탈로그 + 알림 정책

> **참고 (Q3/Q8 결정)**: 별도 notification-service는 폐기. 모든 시스템 알림은 Prometheus metrics → AlertManager → Slack 경로로 처리 (§10 참조). 비즈니스 이벤트(`order.confirmed`/`order.cancelled`)는 발행은 유지하되 현재 consumer 없음 — 추후 다른 서비스가 subscribe할 수 있도록 토픽 자체는 정의 + provisioning.

### 9.1 토픽 카탈로그 (Q5 결정 — SPEC 표준 명명)

| 토픽 | Producer | Consumer | 흐름 |
|------|----------|----------|------|
| `order.pending`            | order-service worker (Outbox poller) | inventory-service | 주문 생성 → 재고 예약 요청 |
| `order.inventory-reserved` | inventory-service | order-service | 재고 예약 결과 회신 |
| `order.confirmed`          | order-service | (현재 없음) | state machine `CONFIRMED` 도달 시 발행 |
| `order.cancelled`          | order-service | (inventory-service: 보상 트랜잭션 시) | state machine `CANCELLED` 도달 시 발행 |

> 모노레포의 `*-topic` suffix(`inventory-reserve-request-topic` 등) 이름은 Phase 4 작업으로 위 표준명으로 변경 (Q5 (a)).

### 9.2 KafkaTopic CRD (Strimzi, Phase 5 platform 작업)

`platform/50-kafka-cluster/topics/order-pending.yaml`:
...
**Kafka wire는 JSON 유지** (Spring Kafka의 `JsonSerializer`/`JsonDeserializer`). 모노레포의 기존 방식 그대로.

`msa-common-libs:events` 의 Protobuf 스키마는 **코드 양립용으로 보유**하되 wire 직렬화에는 미사용. 이유:
- Phase 3 범위 폭주 방지 + 양쪽 서비스(order + inventory) 동시 변경 비용 회피
- 도메인 DTO ↔ JSON 매퍼는 이미 작성 완료
- Protobuf wire는 SemVer breaking change로 별도 트랙에서 진행 (BACKLOG R-13)

### 9.4 DLQ

각 서비스의 Spring Kafka 설정에서 `DeadLetterPublishingRecoverer`로 처리. DLQ 토픽 명명 규칙: `<원토픽>.dlq` (예: `order.pending.dlq`). Phase 5에서 토픽 provisioning과 함께 추가.
...
### 12.0 멀티모듈 구조 (v0.3.0부터)

| 서브모듈 | groupId:artifactId | 내용 |
|---------|---------------------|------|
| `common` | `com.troica.msa:common` | JPA/QueryDSL config, Base 엔티티, 공통 예외/시간/Backoff 유틸 |
| `events` | `com.troica.msa:events` | Kafka 토픽 Protobuf 스키마 + codegen (현재는 코드 양립용, wire는 JSON) |

**v0.2.0에서 제거된 모듈** (D2/D3 결정):
- ~~`client-redis`~~ — 팀장님의 JitPack 패키지 `com.github.kanei0415:ktcloud-msa-client-redis:v1.0.2`로 일원화
- ~~`client-ses`~~ — notification-service 폐기와 함께 사용처 0이라 제거

소비자(서비스 레포)는 필요한 모듈만 의존:
```kotlin
// 모든 서비스
implementation("com.troica.msa:common:0.3.0")

// Kafka 사용 서비스 (order, inventory)
implementation("com.troica.msa:events:0.3.0")

// Redis 분산락/멱등 필요 서비스 (inventory, auth) — JitPack
implementation("com.github.kanei0415:ktcloud-msa-client-redis:v1.0.2")
```

소비자 `repositories`에 JitPack 추가 필요:
```kotlin
...
### Phase 4: 나머지 서비스 복제 + Q5/Q6/Q7 신규 구현

Phase 3에서 검증된 패턴으로 나머지 서비스. 자세한 액션 레시피는 [`PHASE_4_RUNBOOK.md`](./PHASE_4_RUNBOOK.md).

| 서비스 | 상태 | 작업 |
|--------|------|------|
| `msa-product-service` | ✅ 완료 (Phase 4 첫 PoC) | — |
| `msa-order-service` | 추가 작업: **Q5 토픽명 정렬** + **Q6 state machine 확장** | `OrderLineItemStatus` enum에 `PAYMENT_PENDING`/`PAID`/`SHIPPING`/`SHIPPED`/`CONFIRMED`/`CANCELLED` 추가. 상태 전이 시 `order.confirmed`/`order.cancelled` 토픽 발행 (Outbox 일관성) |
| `msa-inventory-service` | 신규 추출 + **Q5 토픽명** + **Q7 Event Sourcing + Spring Batch worker** | `inventory-event`를 append-only 이벤트 스토어로 변환. `@EnableBatchProcessing` + `Job`/`Step` 정의. worker 프로파일 + 별도 K8s Deployment. `client-redis`는 JitPack 의존성 사용 (D2) |
| `msa-user-service` | 신규 추출 | 모노레포 `user/` 단독. `@SpringBootApplication` + REST/gRPC adapter 작성. identification은 폐기 |
| `msa-auth-service` | 신규 추출 (신규 레포 — D1) | 모노레포 `auth/` + `auth-service/`. gRPC `AuthService.{SignUp, SignIn, CheckValidity}` 노출. JWT 발급/검증 |
| `msa-api-gateway` | 신규 추출 + **Q1 (c) 혼합** | 모노레포 `user-api-gateway/`. BFF REST controllers + Spring Cloud Gateway routes + `JwtHeaderCheckFilter` (auth-service gRPC 호출). Spring Cloud BOM 2025.0.2 |
| ~~`msa-notification-service`~~ | **폐기 (D6 archive)** | 비즈니스 이벤트는 토픽 발행만, consumer 없음. 시스템 알림은 §10 AlertManager → Slack |

각 서비스마다 (1) 코드 이전 / 신규 작성, (2) Dockerfile, (3) CI workflow, (4) `applications/values/<service>/` 추가, (5) 로컬 빌드 + bootJar MANIFEST 검증, (6) PR 머지.
