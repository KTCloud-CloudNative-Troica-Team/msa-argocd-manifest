...
| R-25 | `auth-service`의 JWT secret 외부화 | Phase 5 (부분 해결) | 모노레포 application.yaml에 평문 secret(`v7S6A9yB2E5H8KcNfUjXnZr4u7x!A%D*G-`). PoC라도 K8s Secret + ExternalSecrets로 분리 필요 | **Phase 1 fix**: `fix/r-25-jwt-secret-placeholder` PR로 fallback을 명백한 placeholder(`DEV_ONLY_INSECURE_REPLACE_ME_*`)로 치환. **Phase 5 정식 해결**: ExternalSecrets Operator + AWS Secrets Manager (R-33/R-35와 묶음) |
...
| R-31 | ~~ArgoCD `directory.include` brace expansion `{a,b,c}` 미동작~~ | 매니페스트 (해결) | ~~`bootstrap/root-app.yaml`의 `directory.include: '{...}'`이 ArgoCD에서 매칭 안 됨 → 4개 파일 중 0개 sync~~ | **해결 완료** — `fix/r-31-root-app-app-of-apps` PR 머지. app-of-apps 정식 패턴: `bootstrap/root-app.yaml`은 path=bootstrap + recurse=false. 자식 3개(`app-projects`/`app-platform`/`app-applications`)는 `bootstrap/apps.yaml` multi-doc으로 분리 |
| R-32 | ~~`ansible/argocd-setup.yaml`의 root-app repo URL이 팀장님 개인 레포~~ | Phase 0 후속 (해결) | ~~`kanei0415/ktcloud-k8s-argocd-manifest` 가리킴 → 우리 팀 매니페스트 미동기화~~ | **해결 완료** — `fix/r-32-argocd-setup-repo-url` PR 머지. inline yaml 제거 + 매니페스트 레포의 `bootstrap/root-app.yaml`을 fetch + apply (단일 진실의 원천) |
...
| R-34 | ~~`destroy-temp.sh` Windows 호환 + git autocrlf 이슈~~ | Phase 0 후속 (해결) | ~~Windows의 git core.autocrlf로 `.sh`가 CRLF 저장 → bash 실패~~ | **해결 완료** — `chore/destroy-temp-powershell` (PowerShell 네이티브) + `chore/gitattributes-lf-enforcement` (.sh/.yaml/.tf 등을 LF 강제) 두 PR 머지로 양방향 안전 |
...
| R-36 | ansible playbook + README 일본어 → 한국어 번역 | 코드 품질 | msa-provisioning에 일본어 14개 파일: `ansible/{k8s-pre-setup,k8s-pkg-install,containerd-setup,master-init,master-cni-setup,master-python-setup,join-master,join-worker,helm-setup,nlb-setup,argocd-setup,traefik-setup,k8s-clear}.yaml` + `README.md`. 팀장님 작성 코드의 `name:` 필드 + 주석이 일본어라 실행 출력 + 코드 리뷰 가독성 저하. 한 PR로 전체 일괄 번역 (의미 보존 + 줄 단위 1:1 매핑) | 해결 완료 — `i18n/ansible-japanese-to-korean` PR 머지 |
| R-37 | `platform/root.yaml`의 brace expansion glob (R-31 동일 패턴) | 매니페스트 (해결) | `directory.include: '{*/application.yaml,*/*/application.yaml}'` 패턴은 R-31에서 발견한 ArgoCD glob 미지원 문제와 동일. 현재 platform/ 하위 application.yaml 없어서 비활성, 그러나 Phase 5에서 cert-manager / istio / kafka 등 추가될 때 자식 0개 생성 버그 발생 예정. `**/application.yaml` doublestar 패턴으로 전환 + 컨벤션 (helm values 등 sync 비대상은 application.yaml 외 이름) | 해결 완료 — `fix/r-37-platform-root-glob` PR |
